...
- Induction Training on R language! For this head to [UNHCR Learn & Connect]
...
<span style='font-size:30px;'> Dashboard are meaningfull only when displaying KPIs!</span>
...
grViz(" 
  digraph CFA {
    ...
    # Decision 
    node [shape = oval, fontname = Lato, fontsize= 90, margin=1]
    ...
    X -> a [label = 'Start', fontsize= 90, penwidth = 15] 
    a -> 1 [label = 'Yes', fontsize= 90, penwidth = 15]  ## PowerBI
    a -> b [label = 'No', fontsize= 90, penwidth = 15]  ## Continue
    
    ## Basic stat
    b -> c [label = 'Yes', fontsize= 90, penwidth = 15] 
    b -> d  [label = 'No', fontsize= 90, penwidth = 15]
    
    ## Polished visuals
    c -> 2 [label = 'Yes', fontsize= 90, penwidth = 15]
    c -> e [label = 'No', fontsize= 90, penwidth = 15]
    
    ## Machine learning
    d -> e [label = 'Yes', fontsize= 90, penwidth = 15]
    d -> 2 [label = 'No', fontsize= 90, penwidth = 15]
    
    ## Quick processing
    e -> 3 [label = 'Yes', fontsize= 90, penwidth = 15]
    e -> f [label = 'No', fontsize= 90, penwidth = 15]
    
    ## Interactivity
    f -> 4 [label = 'Yes', fontsize= 90, penwidth = 15]
    f -> 2 [label = 'No', fontsize= 90, penwidth = 15]
  }
...
<span style='font-size:30px;'>Reproducibility allows for __peer review__</span>
 ...
 > _It's okay to make mistakes, as long as one can detect them and that you can learn from them..._ 
...
<span style='font-size:30px;'>Automation though functions & scripts can help skipping __repetitive tasks__</span>
...
Many users can use focused R functions through dedicated Graphical User Interface (no need to learn R code...)
...
What is the most appealing exercise among the 2 proposed aside?!! 
...
### Sourcing,  documentating, & packaging
 ...
 * level 2: organise multiple command together in re-usable function
 * Level 3: organise multiple functions together in a package
 * Level 4: Unit testing 
 * Level 5: includes test data & Documentation
]
.pull-right[
...
.pull-left[
<span style='font-size:30px;'>Not all data can call for interpretation - select the one that can gemerate discussions & __debates__</span>


Chart title framed as an opening question
...
> Try to start by using existing packages and start from a project you can reproduce
...
Using Multi Tier IT Standard Support model...
 
  * Tier 4: Code Review & Quality Insurance / Contracted Company with global frame agreement (cost: 250 $/Hour)

  * Tier 3: Internal package development / Internal R champions team (cost: one yearly Rdev meeting to incentivize contributing staff – less than 10 persons)
...
Please fill this survey to empower UNHCR R community
